  ...
  <div>
      ...
      <el-form :model="articleInfo" ref="articleInfo">
        ...
        <el-form-item label="" style="height: 90px;margin-left: 100px;margin-top: 100px;">
          ...
          <el-button type="success" @click="submit('articleInfo')">提交审核</el-button>
...
export default {
    ...
    methods: {
      init() {
        this.articleInfo.userId = this.id
      ...
      saveArticle() {
        articleApi.addArticle(this.articleInfo)
          .then(response =>{
            this.$message({
              type: 'success',
              message: '提交成功！'
      ...
      checkTagAndCover() {  //检验图片上传和标签
        ...
        if(!this.hasCover) {
            ...
            return false
          }else if(this.dynamicTags.length == 0) { //标签内容判断
            this.$message({
                type: 'error',
                 message: '请添加至少一个文章标签!'
            });
