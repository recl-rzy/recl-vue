...
export default {
  ...
  methods: {
    ...
    getMessage (msg) {
      ...
      this.receiveUserStatus = jsonObject.receiveUserStatus
      console.log(this.receiveUserStatus)
      if(!this.receiveUserStatus) {

        this.$message({
            type: 'error',
            message: '当前咨询师处于离线状态,请耐心等待'
        })
      } else {
        this.$message({
            type: 'success',
    ...
    getChatMsg(sendUserId, receiveUserId) {
      chatApi.getChatMsg(sendUserId, receiveUserId)
          .then(response => {
                  this.chatMsg = response.data.data.chatMsgs
